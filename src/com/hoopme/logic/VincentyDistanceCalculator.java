...
public class VincentyDistanceCalculator {
    ...
    public Distance distance(Position p1, Position p2) {
        ...
        double[] ret = gpnhri(ellipsoid.axis, ellipsoid.flat, p1.getLatRads(), p1.getLonRads(),
    ...
    public Position offset(Position p1, Distance distance, Azimuth azimuth) {
        Ellipsoid ellipsoid = getEllipsoid();
        double[] res = dirct1(p1.getLatRads(), p1.getLonRads(), azimuth.getRadians(),
                distance.getMetres(), ellipsoid.axis, ellipsoid.flat);
     ...
     * 
     * @param glat1
     *            The latitude of the starting point, in radians, positive
     *            north.
     * @param glon1
     *            The latitude of the starting point, in radians, positive east.
     * @param azimuth
     *            The azimuth to the desired location, in radians clockwise from
     *            north.
     * @param dist
     ...
     */
    private static double[] dirct1(double glat1, double glon1, double azimuth, double dist,
     ...
     */
    private static double[] gpnhri(double a, double f, double p1, double e1, double p2, double e2) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            double ao = f - f2 * (1.0 + f + f2) * w / 4.0 + 3.0 * f3 * (1.0 + 9.0 * f / 4.0) * t4
                    / 16.0 - 25.0 * f4 * t6 / 128.0;
            double a2 = f2 * (1 + f + f2) * w / 4.0 - f3 * (1 + 9.0 * f / 4.0) * t4 / 4.0 + 75.0
                    * f4 * t6 / 256.0;
        ...
        double z = epsq * w;
        double bo = 1.0 + z
                * (1.0 / 4.0 + z * (-3.0 / 64.0 + z * (5.0 / 256.0 - z * 175.0 / 16384.0)));
     ...
     */
    private static double[] gpnarc(double amax, double flat, double esq, double p1, double p2) {
...
class Position {
    ...
    public static Position fromDegrees(double latDegrees, double lonDegrees) {
        if (Double.isNaN(latDegrees) || Double.isInfinite(latDegrees) || Double.isNaN(lonDegrees)
                || Double.isInfinite(lonDegrees))
            throw new IllegalArgumentException("Components of a Position" + " must be finite");
    ...
    public double getCentreDistance() {
        if (Double.isNaN(latitudeR))
            return Double.NaN;
        double ρ = 0.9983271 + 0.0016764 * cos(2 * latitudeR) - 0.0000035 * cos(4 * latitudeR);
        return ρ;
    }
